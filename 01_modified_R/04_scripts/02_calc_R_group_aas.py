...
def main():
    ...
    for pref in dir_list:
        ...
        group_list = get_mrca_node_number(tree)
...
def get_mrca_node_number(tree):
    group_list =[]
    tw_list = ['Physeter_catodon','Lipotes_vexillifer', 'Delphinapterus_leucas', 'Tursiops_truncatus', 'Orcinus_orca']
    bat1_list = ['Miniopterus_natalensis','Eptesicus_fuscus','Myotis_davidii','Myotis_brandtii','Myotis_lucifugus']
    bat2_list = ['Hipposideros_armiger', 'Rhinolophus_sinicus', 'Rousettus_aegyptiacus']
    leaf_tw = []
    for leaf in tree.iter_leaves():
        if leaf.name in tw_list:
            leaf_tw.append(leaf.name)
    tw_ancnode_num = tree.get_common_ancestor(leaf_tw).name

    leaf_bat1 = []
    for leaf in tree.iter_leaves():
        if leaf.name in bat1_list:
            leaf_bat1.append(leaf.name)
    bat1_ancnode_num = tree.get_common_ancestor(leaf_bat1).name
    group_list.append([tw_ancnode_num,bat1_ancnode_num])

    leaf_bat2 = []
    for leaf in tree.iter_leaves():
        if leaf.name in bat2_list:
            leaf_bat2.append(leaf.name)
    bat_all = leaf_bat1 + leaf_bat2
    bat2_ancnode_num = tree.get_common_ancestor(bat_all).name
    group_list.append([tw_ancnode_num,bat2_ancnode_num])
    return group_list
